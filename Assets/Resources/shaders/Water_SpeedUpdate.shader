...
Shader "Marvel/FX/Water/Speed Update"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // Sample textures
                float speed = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, IN.uv).r;
                ...
                float heightDiff = height * 2.0 - 1.0;
                float impulseDiff = saturate(impulse * 2.0 - 1.0);
                float averageDiff = average * 2.0 - 1.0;
                
                float speedDiff = heightDiff + impulseDiff;
                speedDiff += averageDiff - speedDiff;
                
                float gravityEffect = averageDiff * _GravityPressure;
                speedDiff -= gravityEffect;
                
                float newSpeed = speedDiff * _Dampening + (speed * 2.0 - 1.0);
                newSpeed *= _Friction;
                
                // Convert back to 0-1 range
                float finalSpeed = newSpeed * 0.5 + 0.5;
                
                return float4(finalSpeed, finalSpeed, finalSpeed, 1.0);
            }
            ENDHLSL
        }
    }
